	...
	const debugProbability = (ev) => {
		...
		if (cat !== '-1') {
			...
			Object.keys(results).map((key) => {
				results[key].percent = (results[key].count / num) * 100;
			});
			const results_sorted = Object.entries(results)
...
<main class="grid-main">
	<div class="container-matrix pt-16">
		<div class="g-app-title-header">
			<div class="title">
				<h2 class="h1"><b>My treasure</b>,</h2>
				<h1 class="h4"><i>RPG Treasure / loot generator </i></h1>
				<span class="help ml-8">Help!</span>
			</div>
			<button
				class="caption-action"
				id="random-pick"
				disabled={$config.status.cells.every((v) => v.enabled === false)}
				on:click={pickRandomItem}>Pick random item!</button
			>
		</div>
		...
		<div class="g-table-container mt-16">
			<table id="table-matrix" class="table-matrix">
				<thead>
					<tr>
						<th class:is-disabled={$config.status.all_cells}
							>All
							<label class="switch" for="chk-all">
								<input
									id="chk-all"
									class="table-activation"
									type="checkbox"
									disabled={$config.status.all_cells}
									bind:checked={$config.status.all_cells}
									on:click={enableAllCells}
								/>
							</label>
						</th>
				...
				<tbody>
					{#each $matrix as row, idx}
						<tr>
							<th
								class:is-disabled={!$config.status.rows[idx]}
								on:click|stopPropagation={() => {
									rowClicked(idx);
								}}
								>{idx + 1}<label class="switch" for="chk-row-{idx}">
									<input
										id="chk-row-{idx}"
										data-row-id={idx}
										class="row-activation"
										type="checkbox"
										bind:checked={$config.status.rows[idx]}
										on:click={() => {
											enableDisableRowCells(idx);
										}}
									/>
								</label>
							</th>
				...
				<fieldset class="mt-12">
					...
					<label
						...
						<input
							...
							on:focus={hl}
							on:blur={hl}
							bind:value={$config.base_jewels_small}
						/>
					</label>
					<label
						>Jewels (row 3)<br />
						<input
							...
							on:focus={hl}
							on:blur={hl}
							bind:value={$config.base_jewels_medium}
						/>
					</label>
					<label
						>Units<br />
					...
					<label class="wide" id="cursed-config">
						<input type="checkbox" id="cursed" bind:checked={$config.include_cursed} /> Include
						<b>cursed</b> items ( † )
					</label>
				</fieldset>
			</details>
	...
	<div class="container-treasure pt-16">
		<div class="g-treasure-title-header">
			...
			<button
				disabled={$shopping_cart.length == 0}
				class="caption-action danger"
				id="clear-results"
				on:click={shoppingCartComponent.clearAll}>Clear treasure</button
			>
		</div>
...
<details class="debug mt-8">
	...
	<fieldset class="mt-12">
		...
		<label>
			Num
			<input id="debug-probability-num" type="number" value="100" />
		</label>
		<label>
