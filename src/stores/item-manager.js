...
const parseItem = (item) => {
	...
	if (item.name.includes('$$')) {
		...
		let new_item = JSON.parse(
			JSON.stringify(getRandomItemFromCategory(item.name.replaceAll('$$', '').trim()))
		);
		...
		if (
			!item.desc &&
			item.hasOwnProperty('_category_tree') &&
			item._category_tree.includes('$$art_item$$')
		) {
			// console.log('Is art item');
			item.desc =
				'%%art_item_desc_quality_work%% quality art piece (%%art_item_desc_age%% period) by %%art_item_desc_renown%% artist. %%art_item_desc_quality_material%% materials, and %%art_item_desc_condition%% condition. %%art_item_desc_size%% size.';
		}
